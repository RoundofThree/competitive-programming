...
pair<ll,ll> cal(vll v) {
   ...
   if (x < 0 && y < 0) {
       ...
       return { 1-t, ans + 1 };
            ...
            ll ans = -INF;
            if (x > 0) { ans = max(ans, N - x); }
            if (y > 0) { ans = max(ans, y); }
...
string solve() {
    ...
    while (1) {
        //cout << d.size() << endl;
        if (d.empty()||(d.front()!=t&&d.back()!=t)) {
            if (t == 0)cout << "O " << d.size() + 1;
            else { cout << "I " << d.size() + 1; }
            return "";
        }
        if (d.front() == t && d.back() == t) { break; }
        if (d.front() == t) { d.pop_front(); }
        else { d.pop_back(); }
